...
check 200 "GET  /api/analytics/fields (uploaded)"   "$BASE/api/analytics/fields?datasetId=$NEW_ID"
...
echo "Explorer"
# The fixture's own columns: c0 id, c1 name, c2 dept, c3 salary, c4 joined, c5 active.
check 200 "GET  /api/explorer/columns"              "$BASE/api/explorer/columns?datasetId=$DATASET_ID"
assert "$(grep -q '"label":"salary"' "$TMP/last.json" && echo true || echo false)" "columns are the file's own headers"
check 200 "GET  /api/explorer/rows"                 "$BASE/api/explorer/rows?datasetId=$DATASET_ID&page=1&pageSize=10"
check 200 "GET  /api/explorer/rows (sorted)"        "$BASE/api/explorer/rows?datasetId=$DATASET_ID&sort=c3&dir=desc"
check 200 "GET  /api/explorer/rows (filtered)"      "$BASE/api/explorer/rows?datasetId=$DATASET_ID&filter=c3:gt:60000&filter=dept:eq:engineering"
assert "$(grep -q '"total":1' "$TMP/last.json" && echo true || echo false)" "filters compare numbers by value and text ignoring case"
check 400 "GET  /api/explorer/rows (bad filter)"    "$BASE/api/explorer/rows?datasetId=$DATASET_ID&filter=c3-gt-10000"
...
check 200 "GET  /api/analytics/fields"              "$BASE/api/analytics/fields?datasetId=$DATASET_ID"
check 404 "GET  /api/analytics/fields?datasetId=9999" "$BASE/api/analytics/fields?datasetId=9999"
check 200 "GET  /api/analytics/run (avg by group)"  "$BASE/api/analytics/run?datasetId=$DATASET_ID&metric=c3&aggregate=avg&dimension=c2"
check 200 "GET  /api/analytics/run (count by date)" "$BASE/api/analytics/run?datasetId=$DATASET_ID&aggregate=count&dimension=c4&bucket=year"
check 400 "GET  /api/analytics/run (text metric)"   "$BASE/api/analytics/run?datasetId=$DATASET_ID&metric=c1&aggregate=sum&dimension=c2"
check 400 "GET  /api/analytics/run (bad aggregate)" "$BASE/api/analytics/run?datasetId=$DATASET_ID&metric=c3&aggregate=mode&dimension=c2"
...
check_other 404 "GET  another account's analytics"       "$BASE/api/analytics/run?datasetId=$DATASET_ID&metric=c3&dimension=c2"
